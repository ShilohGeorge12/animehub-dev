...
animeRouter.get('/animes', Auth, tryCatch(async (req, res) => {
...
animeRouter.get('/search', Auth, tryCatch(async (req, res) => {
...
animeRouter.get('/animes/:id', Auth, tryCatch(async (req, res) => {
    const { id } = req.params;
...
animeRouter.post('/animes', Auth, upload.single('image'), tryCatch(async (req, res) => {
    var _a, _b;
    ...
    const anime = new AnimeModel({
        ...
        status,
        image: {
            data: fs.readFileSync('dist/uploads/' + ((_a = req.file) === null || _a === void 0 ? void 0 : _a.filename)),
